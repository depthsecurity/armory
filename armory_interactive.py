...
from armory import print_banner, get_config_options, get_modules, load_module, initialize_database
import readline
import sys
import pdb
import glob
...
class GlobalCommands(object):
    ...
    def __init__(self, name="Armory"):
        self.cmd = {}
        self.name = name
        self.cmd['help'] = {'func':self.show_help,
                        'help':"Displays help text",
                        }

        self.cmd['exit'] = {'func':self.exit_app,
                        'help':"Exits Armory"}

        
        self.cmd['back'] = {'func':self.go_back,
                        'help':"Move back one level"}
...
class MainCommands(GlobalCommands):
    ...
    def __init__(self, name="Armory"):
        ...
        self.cmd['list_modules'] = {'func':self.list_modules,
                                    'help':"List available modules"}

        self.cmd['list_reports'] = {'func':self.list_reports,
                                    'help':"List available reports"}

        self.cmd['use_module'] = {'func':self.use_module,
                                    'help':"Use a module"}

        self.cmd['use_report'] = {'func':self.use_report,
                                    'help':"Use a report"}
...
class ModuleCommands(GlobalCommands):
    ...
    def __init__(self, name=None):
        ...
        self.cmd['options'] = {'func':self.view_options,
                        'help':'Display available options'}

        self.cmd['set'] = {'func':self.set,
                        'help':'Set value for an option'}

        self.cmd['unset'] = {'func':self.unset,
                        'help':'Unset value for an option'}

        self.cmd['run'] = {'func':self.run_module,
                        'help':'Run the module/report'}

        self.cmd['reset'] = {'func':self.reset_module,
                        'help':'Reset the options back to default'}

        self.reset_module()
        

    def reset_module(self, options=None, module_data=['custom_modules', 'included.modules.']):
        config = get_config_options()
        ...
        for a in self.m.options._actions:
            ...
            if atype != "_HelpAction":
                ...
                self.options[o] = {'arg':a.option_strings[-1],
                                    'help':a.help,
                                    'type':atype,
                                    'required':a.required}
    ...
    def view_options(self, options=None):
        ...
        print("{:<25} | {:<10} | {:<20} | {:<40}".format("Field", "Required", "Value", "Description"))
        print("-" * 95)
        for c in options:

            print("{:<25} | {:<10} | {:<20} | {:<40}".format(c, str(self.options[c]['required']), str(self.options[c]['value']), self.options[c]['help']))
        print('\n\n')
            ...
            if options.count(' ') > 0:
                ...
                if self.options[option]['type'] == '_StoreTrueAction':
                    if val.lower() in 'true':
                        ...
                        print("%s is too ambiguous - it should be either True or False" % val)
                else:
                    self.options[option]['value'] = val
            else:
                print("You need to supply a value to set! Use unset to remove all values.")
        else:
...
class ReportCommands(ModuleCommands):
    ...
    def reset_module(self, options=None):
        ...
        super(ReportCommands, self).reset_module(module_data=['custom_reports', 'included.reports.'])
    ...
    def complete(self, text, state):
        ...
        if state == 0:  # on first trigger, build possible matches
            ...
            if readline.get_line_buffer().startswith('use_module '):
                ...
                self.matches = [s for s in self.modules 
                                    if s and s.startswith(text)]
            elif readline.get_line_buffer().startswith('use_report '):
                
                self.matches = [s for s in self.reports 
                                    if s and s.startswith(text)]                        
    ...
    def complete(self, text, state):
        ...
        if state == 0:  # on first trigger, build possible matches
            ...
            if full_cmd.startswith("set "):
                if full_cmd.count(' ') == 1:
                    self.matches = [s for s in self.module_options if s and s.startswith(text)]
                else:
                    # print("\n%s\n%s" % (full_cmd, text))
