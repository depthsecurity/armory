...
def get_hosts(request):
    ...
    # pdb.set_trace()
    host_html = loader.get_template("host_summary/host_summary_data.html").render(
        {
            "ips": good_ips,
            "data": data,
    ...
    sidebar_html = loader.get_template("host_summary/sidebar.html").render(
        {"ips": good_ips}
    )

    return HttpResponse(
        json.dumps({"hostdata": host_html, "sidebardata": sidebar_html})
    )
...
def get_nessus(request, port_id):
    ...
    return render(
        request, "host_summary/nessus.html", {"vulns": vulns_obj, "sev_map": sev_map}
    )
...
def get_ffuf(request, port_id):
    max_status = int(request.GET.get("max_status", 500))
    ...
    for f in port.meta["FFuF"]:
        if os.path.exists(f):
            ...
            for r in data["results"]:
                ...
                if (
                    len(ffuf_data[r["host"]][url][r["status"]])
                    < max_status
                    and r
                    not in ffuf_data[r["host"]][url][r["status"]]
                ):
