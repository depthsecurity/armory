    ...
    if lookup_domains:
        for ip in sorted(ips.keys()):
            ...
            try:
                ...
                if domains:
                    results.append("%s / %s: %s" % (ip, ', '.join(sorted(domains)), ", ".join(["%s/%s/%s" % (proto, p, svc) for p, svc in sorted(ips[ip]['ports'])])))
                else:
                    results.append("%s / No Hostname Registered: %s" % (ip, ", ".join(["%s/%s/%s" % (proto, p, svc) for p, svc in sorted(ips[ip]['ports'])])))
            except:
                if ips[ip]['domains']:
                    results.append("%s / %s: %s" % (ip, ', '.join(sorted(ips[ip]['domains'])), ", ".join(["%s/%s/%s" % (proto, p, svc) for p, svc in sorted(ips[ip]['ports'])])))
                else:
                    results.append("%s / No Hostname Registered: %s" % (ip, ", ".join(["%s/%s/%s" % (proto, p, svc) for p, svc in sorted(ips[ip]['ports'])])))
