...
class Module(ModuleTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('-sm', '--status_match', help="Status Codes that signify valid response")
        self.options.add_argument('-sn', '--status_no_match', help="Status Codes that signify invalid response")
        self.options.add_argument('-p', '--protocol', help='Protocol (http, https, or any) (Default: any)', default="any")
        self.options.add_argument('-rm', '--regex_match', help="Regex on response to indicate valid response")
        self.options.add_argument('-rn', '--regex_no_match', help="Regex on response to indicate invalid response")
        self.options.add_argument('-t', '--timeout', help='Connection timeout (default 5)', default="5")
        self.options.add_argument('-o', '--output', help="Output file (default UrlScanner-<tiemstamp>.txt")
        ...
        self.options.add_argument('-e', '--endpoint', help="The endpoint to attack (default '/')", default="/")
    def run(self, args):
        ...
        if args.output:
            f = open(os.path.join(self.base_config['PROJECT']['base_path'], args.output), 'w')

        else:
            f = open(os.path.join(self.base_config['PROJECT']['base_path'], "UrlScanner-%s" % time.strftime("%Y%m%d-%H%M%S")), 'w')
        ...
        for url in urls:
            ...
            if args.protocol == "any" or args.protocol == proto:
                ...
                try:
                    res = requests.get(url, timeout=int(args.timeout), verify=False, allow_redirects=False)
                    matched = True
                    ...
                    if args.regex_no_match and re.findall(res.text, args.regex_no_match):
