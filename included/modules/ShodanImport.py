...
class Module(ModuleTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('-s', '--search', help="Custom search string (will use credits)")
        ...
        self.options.add_argument('--fast', help="Use 'net' filter. (May use credits)", action="store_true")
        self.options.add_argument('--cidr_only', help="Import only CIDRs from database (not individual IPs)", action="store_true")
        ...
        if args.import_db:
            ...
            if args.rescan:
                ...
                if not args.cidr_only:
                    ranges += ["{}".format(i.ip_address) for i in self.IPAddress.all(scope_type="active")]
            else:
                if args.fast:
                    ...
                    for c in cidrs:
                        ranges += [str(i) for i in IPNetwork(c)]
                if not args.cidr_only:
                    ranges += ["{}".format(i.ip_address) for i in self.IPAddress.all(scope_type="active", tool=self.name)]
        
        
        api_host_url = "https://api.shodan.io/shodan/host/{}?key={}"
        api_search_url = "https://api.shodan.io/shodan/host/search?key={}&query={}&page={}"
        for r in ranges:
            ...
            if ':' in r:
                ...
                try:
                    results = json.loads(requests.get(api_search_url.format(args.api_key, r, 1)).text)
                    if results.get('error') and 'request timed out' in results['error']:
                        display_warning("Timeout occurred on Shodan's side.. trying again in 5 seconds.")
                        results = json.loads(requests.get(api_search_url.format(args.api_key, r, 1)).text)
                ...
                while total > 0:
                    ...
                    try:
                        time.sleep(1)
                        results = json.loads(requests.get(api_search_url.format(args.api_key, r, i)).text)
                        if results.get('error') and 'request timed out' in results['error']:
                            display_warning("Timeout occurred on Shodan's side.. trying again in 5 seconds.")
                            results = json.loads(requests.get(api_search_url.format(args.api_key, r, 1)).text)
                ...
                for res in matches:
                    ...
                    display("Processing IP: {} Port: {}/{}".format(ip_str, port_str, transport))
                ...
                try:
                    results = json.loads(requests.get(api_host_url.format(r, args.api_key)).text)
                except Exception as e:
                    display_error("Something went wrong: {}".format(e))
                    next
                # pdb.set_trace()
