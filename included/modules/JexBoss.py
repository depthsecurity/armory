...
class Module(ToolTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('--group_size', help="How many hosts per group (default all urls in same group)", type=int, default=0)
    ...
    def get_targets(self, args):
        ...
        if args.output_path[0] == "/":
            self.path = os.path.join(self.base_config['PROJECT']['base_path'], args.output_path[1:], timestamp )
        else:
            self.path = os.path.join(self.base_config['PROJECT']['base_path'], args.output_path, timestamp )
        ...
        for url_chunk in self.chunks(targets, args.group_size):
            ...
            res.append({'target':file_name, 'output':self.path + "-results-{}.txt".format(i)})
