...
from database.repositories import BaseDomainRepository, DomainRepository, IPRepository, PortRepository, VulnRepository, CVERepository, ScopeCIDRRepository
...
class Module(ModuleTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('--launch', help='Launch Nessus scan using Actively scoped IPs and domains in the database', action="store_true")
        self.options.add_argument('--job_name', help="Job name inside Nessus", default="Armory Job")
        self.options.add_argument('--username', help='Nessus Username')
        self.options.add_argument('--password', help='Nessus Password')
        self.options.add_argument('--host', help='Hostname:Port of Nessus web interface (ie localhost:8835')
        ...
        self.options.add_argument('--download', help="Download Nessus job from server and import", action="store_true")
        self.options.add_argument('--job_id', help="Job ID to download and import")
        self.options.add_argument('--output_path', help='Path to store downloaded file (Default: Nessus)', default=self.name)
    ...
    def run(self, args):
        if args.import_file:
            ...
            if not args.username and not args.password and not args.host and not args.uuid and not args.policy_id and not args.folder_id:
                display_error("You must supply a username, password, and host to launch a Nessus job")

            else:
                n = NessusRequest(args.username, args.password, args.host, uuid=args.uuid, policy_id=args.policy_id, folder_id=args.folder_id)

                ips = [ip.ip_address for ip in self.IPAddress.all(scope_type="active", tool=self.name)]
                cidrs = [cidr.cidr for cidr in self.ScopeCIDR.all(tool=self.name)]
                domains = [domain.domain for domain in self.Domain.all(scope_type="active", tool=self.name)]
                ...
                display("Remember this number! You'll need it to download the job once it is done.")

        elif args.download:
            if not args.username and not args.password and not args.host and not args.job_id:
                display_error("You must supply host, username, password and job_id to download a report to import")

            else:
                n = NessusRequest(args.username, args.password, args.host, proxies={'https':'127.0.0.1:8080'})
                ...
                output_path = os.path.join(output_path, 'Nessus-export-{}.nessus'.format(int(time.time())))
    ...
    def process_data(self, nFile, args):
        ...
        for ReportHost in root.iter('ReportHost'):
            ...
            if hostIP:      #apparently nessus doesn't always have an IP to work with...
                ...
                if hostname:
                    display("Gathering Nessus info for {} ( {} )".format(hostIP,hostname))
                else:
