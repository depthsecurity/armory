...
class Module(ModuleTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('-c', '--command', help="Print out a list of commands to run", action="store_true")
        self.options.add_argument('-ps', '--powershell', help="Filename for a .ps1 command output")
        self.options.add_argument('-i', '--import_file', help="import CSV file generated by PowerMeta")
        self.options.add_argument('-f', '--import_folder', help="import CSV files generated by PowerMeta")
    ...
    def run(self, args):
        ...
        if args.command:
            ...
            res += ["Invoke-PowerMeta -TargetDomain %s -Download -Extract -ExtractAllToCsv %s.csv" % (b.domain, b.domain) for b in self.BaseDomain.all()]
            ...
            if d.strip():
                ...
                if res and res[0].lower() == "y":
                    if ' ' in d:
                        ...
                        created, user = self.User.find_or_create(first_name=first_name, last_name=last_name)
                        if created:
