...
class Module(ModuleTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('-b', '--binary', help="Path to binary for LinkedInt", default=self.binary_name)
        ...
        self.options.add_argument('-e', '--email_format', help="Format for emails: auto,full,firstlast,firstmlast,flast,first.last,fmlast,lastfirst, default is auto", default="auto")
        ...
        self.options.add_argument('--smart_shuffle', help="Provide a list of keywords. The tool will run once with all of the keywords, then run again excluding all of the keywords. This is useful for bypassing the 1k limit. Keywords must be comma separated.")
        self.options.add_argument('--top', help="Use the top X keywords from the job titles for smart shuffle")
    ...
    def run(self, args):
        ...
        if not self.binary:
            display_error("LinkedInt binary not found. Please explicitly provide path with --binary")
        ...
        if args.domain:
            ...
            if args.smart_shuffle:
                args.keywords = " OR ".join(['"{}"'.format(i) for i in args.smart_shuffle.split(',')])
                self.process_domain(domain, args)
                args.keywords = " AND ".join(['-"{}"'.format(i) for i in args.smart_shuffle.split(',')])
                self.process_domain(domain, args)
            else:
                self.process_domain(domain, args)
