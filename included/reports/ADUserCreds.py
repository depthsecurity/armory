...
class Report(ReportTemplate):
    ...
    def set_options(self):
        ...
        self.options.add_argument('-ih', '--hashes', help="Input ntds export containing hashes")
        self.options.add_argument('-ic', '--cracked', help="Input file containing cracked hashes (hash:plaintext)")

        self.options.add_argument('-o1', '--user_passwords', help="Report of all user names with plaintext passwords.", action="store_true")                
        self.options.add_argument('--columns', help="Print out user/passwords by columns (Useful for screenshots)", action="store_true")
        self.options.add_argument('-o2', '--lm_hashes', help="export of all lines containing lm hashes", action="store_true")
        self.options.add_argument('-o3', '--password_audit', help='Display a password audit of the compromised data', action="store_true")
        self.options.add_argument('-d', '--delimiter', help='Delimiter for output of user passwords (Default :)', default=":")
        self.options.add_argument('-k', '--keywords', help='Additional keywords to search for the export report (Comma separated')
    ...
    def run(self, args):
        ...
        if args.user_passwords:
            ...
            if cracked_data[:2] == b"\xff\xfe":
                cracked_data = cracked_data[2:].replace(b'\x00', b'').replace(b'\r', b'')
                    ...
                    try:
                        ...
                        if hash_map.get(h, False):
                            if not pw_count.get(hash_map[h], False):
                                pw_count[hash_map[h]] = {'count':0, 'length':len(hash_map[h])}
            ...
            total_cracked_unique = int(total_pws_unique/float(total_hashes_unique)*1000)/10.0

            results.append("%s%sPassword Hashes" % (total_hashes,td))
            results.append("%s%sUnique Password Hashes" % (total_hashes_unique,td))
            results.append("%s%sPasswords Discovered Through Cracking" % (total_pws,td))
            results.append("%s%sUnique Passwords Discovered Through Cracking" % (total_pws_unique,td))
            results.append("%s%sPercent of Passwords Cracked" % (total_cracked,td))
            results.append("%s%sPercent of Unique Passwords Cracked" % (total_cracked_unique,td))
            ...
            years = [str(y) for y in range(datetime.datetime.now().year, datetime.datetime.now().year-15, -1)]
            

            results.append("%s%sPasswords Containing a Season" % (self.search_term(seasons, pw_count), td))
            results.append("%s%sPasswords Containing a Month" % (self.search_term(months, pw_count), td))
            results.append("%s%sPasswords Containing a Year" % (self.search_term(years, pw_count), td))
