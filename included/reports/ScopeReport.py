...
from database.repositories import IPRepository, BaseDomainRepository, DomainRepository, CIDRRepository
import pdb
import json
...
class Report(ReportTemplate):
    ...
    def run(self, args):
        ...
        for b in base_domains:
            results.append('%s\tActive Scope: %s\tPassive Scope: %s' % (b.domain, b.in_scope, b.passive_scope))

            for d in b.subdomains:
                results.append('\t%s\tActive Scope: %s\tPassive Scope: %s' % (d.domain, d.in_scope, d.passive_scope))
        ...
        for c in cidrs:
            ...
            for ip in c.ip_addresses:
                results.append('\t%s\tActive Scope: %s\tPassive Scope: %s' % (ip.ip_address, ip.in_scope, ip.passive_scope))
